...
### 4.1 Resource-Constrained mode (paper-inspired)

This repo now includes a lightweight mode adapted from the ideas in:
[`FAST-LIVO2 on Resource-Constrained Platforms`](https://arxiv.org/pdf/2501.13876).

For Jetson Orin Nano Super 8G / Mid-360 style deployment:

```bash
roslaunch fast_livo mapping_mid360_orin_super8g.launch
```

The launch file uses:
- `config/mid360_orin_super8g_resource.yaml`
- `rviz_cfg/fast_livo2_edge.rviz`

Main additions in this mode:
- LiDAR-degeneration-aware adaptive visual frame selection.
- Hybrid visual map (local unified map + long-term sparse visual map).
- Publish-side load reduction for smoother RViz real-time display.

Important:
- Replace `extrin_calib` in the YAML with your real calibration.
- Tune `adaptive_visual_update/*` and `visual_map/*` thresholds based on your motion profile.
- If you need RGB cloud output, set `publish/vio_colorize_en: true` (higher CPU cost).
