...
public:
  ...
  bool shouldRunVisualUpdate(double img_time);
  double rotationDeltaRad(const M3D &R_ref, const M3D &R_cur) const;
  ...
  int pub_scan_num = 1;
  bool vio_publish_en = true;
  bool vio_colorize_en = true;
  int max_publish_points = -1;
  double publish_voxel_leaf_size = 0.0;

  bool adaptive_visual_update_en = false;
  double keyframe_trans_thresh_m = 1.0;
  double keyframe_rot_thresh_deg = 60.0;
  double keyframe_scale_gain = 1.0;
  double keyframe_scale_min = 0.0;
  double keyframe_scale_max = 1.0;
  int max_visual_skip_frames = 6;
  bool force_visual_when_degenerate = true;
  bool has_visual_keyframe = false;
  int visual_skip_count = 0;
  int visual_update_count = 0;
  int visual_skip_total = 0;
  V3D last_visual_keyframe_pos = V3D::Zero();
  M3D last_visual_keyframe_rot = M3D::Identity();
  double last_visual_keyframe_time = -1.0;

  bool hybrid_visual_map_en = false;
  double visual_voxel_size = 0.5;
  double visual_local_map_edge_length = 200.0;
  double visual_long_term_map_edge_length = 800.0;
  double visual_local_map_slide_thresh = 20.0;
  double visual_long_term_map_slide_thresh = 100.0;
  int visual_local_points_per_voxel = 30;
  int visual_long_term_points_per_voxel = 2;
  int visual_max_obs_size_per_point = 30;
  int visual_max_append_points_normal = 600;
  int visual_max_append_points_degenerate = 1200;
  int visual_max_long_term_obs_per_point = 1;
  bool visual_compact_long_term_observations = true;
  int visual_map_stat_log_interval = 20;
