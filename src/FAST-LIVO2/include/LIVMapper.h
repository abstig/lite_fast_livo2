...
public:
  ...
  void savePCD();
  void processImu();
  void trimBufferedDataLocked();
  void limitPointCloud(PointCloudXYZI::Ptr &cloud, int max_points) const;
  ...
  int lidar_queue_size = 8, imu_queue_size = 128, img_queue_size = 4;
  int image_stride = 1, image_counter = 0;
  int max_lio_points = -1, max_pub_points = -1;
  int max_visual_points = -1, max_new_points = -1, max_obs_per_point = 30, update_ref_patch_stride = 1, omp_num_threads = 1;
  size_t max_lidar_buffer_size = 6, max_imu_buffer_size = 400, max_img_buffer_size = 4, max_prop_imu_buffer_size = 400;
  double max_image_delay = 0.05, max_sync_gap = 0.05, visual_voxel_size = 0.5;
  float min_shi_score = 0.0f;
  bool low_latency_mode = false, verbose_log = true, tracking_img_en = true, subscriber_tcp_nodelay = true;
