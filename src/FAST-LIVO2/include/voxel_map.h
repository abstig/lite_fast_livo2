...
typedef struct VoxelMapConfig
{
  ...
  // config of LiDAR degeneration evaluation
  double lidar_degeneracy_threshold;
  int lidar_degeneracy_consecutive_frames;
} VoxelMapConfig;
...
public:
  ...
  int lidar_degenerate_consecutive_count_ = 0;
...
private:
  void evaluateLidarDegeneracy(const std::vector<PointToPlane> &ptpl_list);
