...
public:
  ...
  // Resource-constrained map and update controls.
  bool hybrid_map_en = false;
  double visual_voxel_size = 0.5;
  double local_map_edge_length = 200.0;
  double long_term_map_edge_length = 800.0;
  double local_map_sliding_thresh = 20.0;
  double long_term_map_sliding_thresh = 100.0;
  int local_points_per_voxel = 30;
  int long_term_points_per_voxel = 2;
  int max_obs_size_per_point = 30;
  int max_append_points_normal = 600;
  int max_append_points_degenerate = 1200;
  int max_long_term_obs_per_point = 1;
  bool compact_long_term_observations = true;
  int map_stat_log_interval = 20;
  bool lidar_is_degenerate_ = false;
  double lidar_sigma_min_norm_ = 1.0;
  V3D last_local_slide_position = V3D::Zero();
  V3D last_long_term_slide_position = V3D::Zero();
  bool local_slide_initialized = false;
  bool long_term_slide_initialized = false;
  ...
  void setLidarDegenerationState(bool is_degenerate, double sigma_min_norm);
  ...
  VOXEL_LOCATION worldPointToVisualVoxel(const V3D &pt_w) const;
  VOXEL_POINTS *findVisualVoxelPoints(const VOXEL_LOCATION &position);
  void addPointToLongTermMap(VisualPoint *pt, const VOXEL_LOCATION &preferred_position);
  void transferLocalVoxelToLongTerm(const VOXEL_LOCATION &position, VOXEL_POINTS *voxel_points);
  void pruneVisualPointObservations(VisualPoint *pt, int max_obs);
  void maintainHybridVisualMap();
  void slideLocalVisualMap(const VOXEL_LOCATION &center_position);
  void slideLongTermVisualMap(const VOXEL_LOCATION &center_position);
  size_t countVisualPoints(const unordered_map<VOXEL_LOCATION, VOXEL_POINTS *> &map) const;
  double evaluatePointQuality(const VisualPoint *pt) const;
