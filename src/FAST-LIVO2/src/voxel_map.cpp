...
void loadVoxelConfig(ros::NodeHandle &nh, VoxelMapConfig &voxel_config)
{
  ...
  nh.param<bool>("lio/enable_neighbor_search", voxel_config.enable_neighbor_search_, true);
  nh.param<int>("runtime/omp_num_threads", voxel_config.omp_num_threads_, 1);
  nh.param<bool>("runtime/verbose_log", voxel_config.verbose_, true);
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr &world_lidar = world_lidar_cache_;
    ...
    if (config_setting_.verbose_ && effct_feat_num_ > 0)
    {
...
void VoxelMapManager::BuildResidualListOMP(std::vector<pointWithVar> &pv_list, std::vector<PointToPlane> &ptpl_list)
{
...
#ifdef MP_EN
  omp_set_num_threads(std::max(1, config_setting_.omp_num_threads_));
  ...
  for (int i = 0; i < static_cast<int>(pv_list.size()); i++)
  {
    ...
    if (!is_sucess && config_setting_.enable_neighbor_search_)
    {
