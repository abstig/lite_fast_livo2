...
void loadVoxelConfig(ros::NodeHandle &nh, VoxelMapConfig &voxel_config)
{
  ...
  nh.param<double>("adaptive_visual_update/lidar_degeneracy_threshold",
                   voxel_config.lidar_degeneracy_threshold, 0.07);
  nh.param<int>("adaptive_visual_update/lidar_degeneracy_consecutive_frames",
                voxel_config.lidar_degeneracy_consecutive_frames, 3);
}
...
void VoxelMapManager::evaluateLidarDegeneracy(const std::vector<PointToPlane> &ptpl_list)
{
  const int required_frames = std::max(1, config_setting_.lidar_degeneracy_consecutive_frames);
  if (ptpl_list.empty())
  {
    lidar_sigma_min_norm_ = 0.0;
    lidar_degenerate_consecutive_count_ += 1;
    lidar_is_degenerate_ = lidar_degenerate_consecutive_count_ >= required_frames;
    return;
  }

  Eigen::Matrix3d translation_info = Eigen::Matrix3d::Zero();
  int valid_normal_count = 0;
  for (const auto &ptpl : ptpl_list)
  {
    Eigen::Vector3d normal = ptpl.normal_;
    const double n_norm = normal.norm();
    if (n_norm < 1e-8) continue;
    normal /= n_norm;
    translation_info += normal * normal.transpose();
    valid_normal_count++;
  }

  if (valid_normal_count < 3)
  {
    lidar_sigma_min_norm_ = 0.0;
    lidar_degenerate_consecutive_count_ += 1;
    lidar_is_degenerate_ = lidar_degenerate_consecutive_count_ >= required_frames;
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> solver(translation_info);
  ...
  Eigen::Vector3d singular_values = solver.eigenvalues().cwiseMax(0.0);
  const double singular_sum = singular_values.sum();
  if (singular_sum < 1e-8) lidar_sigma_min_norm_ = 0.0;
  else lidar_sigma_min_norm_ = singular_values(0) / singular_sum;

  if (lidar_sigma_min_norm_ < config_setting_.lidar_degeneracy_threshold)
  {
    lidar_degenerate_consecutive_count_ += 1;
  }
  else
  {
    lidar_degenerate_consecutive_count_ = 0;
  }
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  evaluateLidarDegeneracy(ptpl_list_);
