...
namespace
{
void limitPointCloudSize(PointCloudXYZI &cloud, const int max_points)
{
  if (max_points <= 0 || static_cast<int>(cloud.size()) <= max_points) return;

  PointCloudXYZI decimated;
  ...
  for (int i = 0; i < max_points; ++i)
  {
    ...
    decimated.push_back(cloud.points[index]);
  }
  cloud.swap(decimated);
}
} // namespace
...
Preprocess::Preprocess() : lidar_type(AVIA), point_filter_num(1), N_SCANS(6), blind(0.01), blind_sqr(0.0), surface_points_cap(-1), feature_enabled(0), given_offset_time(false), verbose(true)
{
  inf_bound = 10;
  group_size = 8;
  disA = 0.01;
...
{
  avia_handler(msg);
  limitPointCloudSize(pl_surf, surface_points_cap);
  pcl_out->swap(pl_surf);
}
...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  if (verbose) printf("[ Preprocess ] Input point number: %d \n", plsize);
  ...
  if (feature_enabled)
  {
    ...
    if (verbose) printf("Feature extraction time: %lf \n", time / count);
  }
  else
  {
    for (uint i = 0; i < plsize; i++)
  ...
  if (verbose) printf("[ Preprocess ] Output point number: %zu \n", pl_surf.points.size());
}
