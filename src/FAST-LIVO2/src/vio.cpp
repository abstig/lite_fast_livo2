...
void VIOManager::setLidarDegenerationState(bool is_degenerate, double sigma_min_norm)
{
  lidar_is_degenerate_ = is_degenerate;
  lidar_sigma_min_norm_ = sigma_min_norm;
}

VOXEL_LOCATION VIOManager::worldPointToVisualVoxel(const V3D &pt_w) const
{
  const double voxel_size = std::max(1e-4, visual_voxel_size);
  int64_t loc_xyz[3];
  for (int j = 0; j < 3; ++j)
  {
    double coord = floor(pt_w[j] / voxel_size);
    if (coord < 0) coord -= 1.0;
    loc_xyz[j] = static_cast<int64_t>(coord);
  }
  return VOXEL_LOCATION(loc_xyz[0], loc_xyz[1], loc_xyz[2]);
}

VOXEL_POINTS *VIOManager::findVisualVoxelPoints(const VOXEL_LOCATION &position)
{
  auto iter = feat_map.find(position);
  if (iter != feat_map.end()) return iter->second;

  auto iter_long = long_term_feat_map.find(position);
  if (iter_long != long_term_feat_map.end()) return iter_long->second;

  return nullptr;
}

double VIOManager::evaluatePointQuality(const VisualPoint *pt) const
{
  if (pt == nullptr) return -1.0;
  double score = static_cast<double>(pt->obs_.size());
  if (pt->is_converged_) score += 4.0;
  if (pt->has_ref_patch_) score += 1.0;
  return score;
}

size_t VIOManager::countVisualPoints(const unordered_map<VOXEL_LOCATION, VOXEL_POINTS *> &map) const
{
  size_t total = 0;
  for (const auto &kv : map)
  {
    if (kv.second == nullptr) continue;
    total += kv.second->voxel_points.size();
  }
  return total;
}

void VIOManager::pruneVisualPointObservations(VisualPoint *pt, int max_obs)
{
  if (pt == nullptr || max_obs <= 0 || pt->obs_.empty()) return;

  if (!pt->has_ref_patch_ || pt->ref_patch == nullptr)
  {
    pt->ref_patch = pt->obs_.front();
    pt->has_ref_patch_ = true;
  }

  while (pt->obs_.size() > static_cast<size_t>(max_obs))
  {
    Feature *victim = nullptr;
    float victim_score = std::numeric_limits<float>::max();
    for (Feature *obs : pt->obs_)
    {
      if (obs == nullptr || obs == pt->ref_patch) continue;
      if (obs->score_ < victim_score)
      {
        victim_score = obs->score_;
        victim = obs;
      }
    }

    if (victim == nullptr) break;
    pt->deleteFeatureRef(victim);
  }
}

void VIOManager::addPointToLongTermMap(VisualPoint *pt, const VOXEL_LOCATION &preferred_position)
{
  if (pt == nullptr) return;

  if (long_term_points_per_voxel <= 0)
  {
    delete pt;
    return;
  }

  pt->in_long_term_map_ = true;
  if (compact_long_term_observations)
  {
    pruneVisualPointObservations(pt, max_long_term_obs_per_point);
  }

  auto iter = long_term_feat_map.find(preferred_position);
  if (iter == long_term_feat_map.end())
  {
    VOXEL_POINTS *voxel_points = new VOXEL_POINTS(0);
    voxel_points->voxel_points.push_back(pt);
    voxel_points->count = 1;
    long_term_feat_map[preferred_position] = voxel_points;
    return;
  }

  VOXEL_POINTS *voxel_points = iter->second;
  ...
  auto &points = voxel_points->voxel_points;
  if (static_cast<int>(points.size()) < long_term_points_per_voxel)
  {
    points.push_back(pt);
    voxel_points->count = static_cast<int>(points.size());
    return;
  }
  ...
  {
    double q = evaluatePointQuality(points[i]);
    if (q < worst_quality)
    {
      worst_quality = q;
      worst_id = i;
  ...
  const double new_quality = evaluatePointQuality(pt);
  if (worst_id >= 0 && new_quality > worst_quality)
  {
    delete points[worst_id];
    points[worst_id] = pt;
  }
  else
  {
...
void VIOManager::transferLocalVoxelToLongTerm(const VOXEL_LOCATION &position, VOXEL_POINTS *voxel_points)
{
  if (voxel_points == nullptr) return;

  for (VisualPoint *pt : voxel_points->voxel_points)
  {
    ...
    addPointToLongTermMap(pt, position);
  }

  voxel_points->voxel_points.clear();
  delete voxel_points;
}

void VIOManager::slideLocalVisualMap(const VOXEL_LOCATION &center_position)
{
  const double voxel_size = std::max(1e-4, visual_voxel_size);
  const int64_t half_range =
      std::max<int64_t>(1, static_cast<int64_t>(0.5 * local_map_edge_length / voxel_size));

  for (auto it = feat_map.begin(); it != feat_map.end();)
  {
    const VOXEL_LOCATION &loc = it->first;
    const bool outside = (std::llabs(loc.x - center_position.x) > half_range) ||
                         (std::llabs(loc.y - center_position.y) > half_range) ||
                         (std::llabs(loc.z - center_position.z) > half_range);
    if (!outside)
    {
      ++it;
      continue;
    ...
    transferLocalVoxelToLongTerm(loc, it->second);
    it = feat_map.erase(it);
  }
}

void VIOManager::slideLongTermVisualMap(const VOXEL_LOCATION &center_position)
{
  const double voxel_size = std::max(1e-4, visual_voxel_size);
  const int64_t half_range =
      std::max<int64_t>(1, static_cast<int64_t>(0.5 * long_term_map_edge_length / voxel_size));

  for (auto it = long_term_feat_map.begin(); it != long_term_feat_map.end();)
  {
    const VOXEL_LOCATION &loc = it->first;
    const bool outside = (std::llabs(loc.x - center_position.x) > half_range) ||
                         (std::llabs(loc.y - center_position.y) > half_range) ||
    ...
    delete it->second;
    it = long_term_feat_map.erase(it);
  }
}

void VIOManager::maintainHybridVisualMap()
{
  if (!hybrid_map_en || new_frame_ == nullptr) return;

  const V3D cur_pos = new_frame_->pos();
  const VOXEL_LOCATION center_position = worldPointToVisualVoxel(cur_pos);

  if (!local_slide_initialized)
  {
    local_slide_initialized = true;
    last_local_slide_position = cur_pos;
  }
  if (!long_term_slide_initialized)
  {
    long_term_slide_initialized = true;
    last_long_term_slide_position = cur_pos;
  }

  if ((cur_pos - last_local_slide_position).norm() >= local_map_sliding_thresh)
  {
    last_local_slide_position = cur_pos;
    slideLocalVisualMap(center_position);
  }

  if ((cur_pos - last_long_term_slide_position).norm() >= long_term_map_sliding_thresh)
  {
    last_long_term_slide_position = cur_pos;
    slideLongTermVisualMap(center_position);
  }

  if (map_stat_log_interval > 0 && new_frame_->id_ % map_stat_log_interval == 0)
  {
    const size_t local_points = countVisualPoints(feat_map);
    const size_t long_term_points = countVisualPoints(long_term_feat_map);
    printf("[ VIO ] Hybrid visual map stats - local voxels: %zu, local points: %zu, long-term voxels: %zu, long-term points: %zu\n",
           feat_map.size(), local_points, long_term_feat_map.size(), long_term_points);
  }
}
...
void VIOManager::insertPointIntoVoxelMap(VisualPoint *pt_new)
{
  ...
  pt_new->in_long_term_map_ = false;

  VOXEL_LOCATION position = worldPointToVisualVoxel(pt_new->pos_);
  auto iter = feat_map.find(position);
  ...
  voxel_points->voxel_points.push_back(pt_new);
  voxel_points->count = static_cast<int>(voxel_points->voxel_points.size());

  if (local_points_per_voxel > 0 && static_cast<int>(voxel_points->voxel_points.size()) > local_points_per_voxel)
  {
    VisualPoint *pt_drop = voxel_points->voxel_points.front();
    voxel_points->voxel_points.erase(voxel_points->voxel_points.begin());
    voxel_points->count = static_cast<int>(voxel_points->voxel_points.size());

    if (hybrid_map_en) addPointToLongTermMap(pt_drop, position);
    else delete pt_drop;
  }
}
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  const float voxel_size = std::max(1e-4f, static_cast<float>(visual_voxel_size));
  ...
  for (auto &iter : sub_feat_map)
  {
    ...
    VOXEL_POINTS *corre_voxel = findVisualVoxelPoints(position);
    // double t5 = omp_get_wtime();

    if (corre_voxel != nullptr)
    {
      bool voxel_in_fov = false;
      std::vector<VisualPoint *> &voxel_points = corre_voxel->voxel_points;
  ...
  if (raycast_en)
  {
    for (int i = 0; i < length; i++)
    {
      ...
      for (const auto &it : rays_with_sample_points[i])
      {
        ...
        VOXEL_POINTS *corre_feat_map = findVisualVoxelPoints(sample_pos);
        if (corre_feat_map != nullptr)
        {
          bool voxel_in_fov = false;

          std::vector<VisualPoint *> &voxel_points = corre_feat_map->voxel_points;
          int voxel_num = voxel_points.size();
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  const int append_limit_cfg = lidar_is_degenerate_ ? max_append_points_degenerate : max_append_points_normal;
  const int append_limit = append_limit_cfg > 0 ? append_limit_cfg : std::numeric_limits<int>::max();
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    if (max_obs_size_per_point > 0 && pt->obs_.size() >= static_cast<size_t>(max_obs_size_per_point))
    {
      Feature *ref_ftr;
    ...
    if (compact_long_term_observations && pt->in_long_term_map_)
    {
      pruneVisualPointObservations(pt, max_long_term_obs_per_point);
    }

    if (patch_temp != nullptr)
    {
      delete[] patch_temp;
    }
  }
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  printf("\033[1;34m+-------------------------------------------------------------+\033[0m\n");
  const size_t local_visual_points = countVisualPoints(this->feat_map);
  const size_t long_term_visual_points = countVisualPoints(long_term_feat_map);
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "Local Visual Voxels", this->feat_map.size());
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "Local Visual Points", local_visual_points);
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "Long-term Visual Voxels", long_term_feat_map.size());
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "Long-term Visual Points", long_term_visual_points);
  printf("\033[1;34m+-------------------------------------------------------------+\033[0m\n");
