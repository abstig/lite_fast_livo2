...
void VIOManager::insertPointIntoVoxelMap(VisualPoint *pt_new)
{
  ...
  double voxel_size = visual_voxel_size;
  float loc_xyz[3];
  for (int j = 0; j < 3; j++)
  {
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  float voxel_size = static_cast<float>(visual_voxel_size);
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      if (max_visual_points > 0 && static_cast<int>(visual_submap->voxel_points.size()) >= max_visual_points) break;
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    if (new_frame_->cam_->isInFrame(pc.cast<int>(), border)) // 20px is the patch size in the matcher
    {
      ...
      if (grid_num[index] != TYPE_MAP)
      {
        ...
        if (cur_value < min_shi_score) continue;
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_POINTCLOUD) // && (scan_value[i]>=50))
    {
      ...
      add += 1;
      if (max_new_points > 0 && add >= max_new_points) break;
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    if (pt->obs_.size() >= max_obs_per_point)
    {
      Feature *ref_ftr;
...
void VIOManager::updateReferencePatch(const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      loc_xyz[j] = p_w[j] / visual_voxel_size;
      if (loc_xyz[j] < 0) { loc_xyz[j] -= 1.0; }
    }
    VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);
...
void VIOManager::updateState(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    #ifdef MP_EN
      omp_set_num_threads(std::max(1, omp_num_threads));
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  if (tracking_img_en || colmap_output_en)
  {
    if (img.channels() == 3) img_rgb = img.clone();
    else cv::cvtColor(img, img_rgb, CV_GRAY2BGR);
  }
  else
  {
    img_rgb.release();
  ...
  if (tracking_img_en)
  {
    if (img.channels() == 3) img_cp = img.clone();
    else cv::cvtColor(img, img_cp, CV_GRAY2BGR);
  }
  else
  {
    img_cp.release();
  ...
  if (tracking_img_en) plotTrackedPoints();
  ...
  if (update_ref_patch_stride <= 1 || (frame_count % update_ref_patch_stride) == 0) updateReferencePatch(feat_map);
