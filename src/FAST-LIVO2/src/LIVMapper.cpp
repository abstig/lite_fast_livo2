...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<bool>("publish/vio_publish_en", vio_publish_en, true);
  nh.param<bool>("publish/vio_colorize_en", vio_colorize_en, true);
  nh.param<int>("publish/max_publish_points", max_publish_points, -1);
  nh.param<double>("publish/publish_voxel_leaf_size", publish_voxel_leaf_size, 0.0);

  nh.param<bool>("adaptive_visual_update/enable", adaptive_visual_update_en, false);
  nh.param<double>("adaptive_visual_update/keyframe_trans_thresh_m", keyframe_trans_thresh_m, 1.0);
  nh.param<double>("adaptive_visual_update/keyframe_rot_thresh_deg", keyframe_rot_thresh_deg, 60.0);
  nh.param<double>("adaptive_visual_update/keyframe_scale_gain", keyframe_scale_gain, 1.0);
  nh.param<double>("adaptive_visual_update/keyframe_scale_min", keyframe_scale_min, 0.0);
  nh.param<double>("adaptive_visual_update/keyframe_scale_max", keyframe_scale_max, 1.0);
  nh.param<int>("adaptive_visual_update/max_skip_frames", max_visual_skip_frames, 6);
  nh.param<bool>("adaptive_visual_update/force_visual_when_degenerate", force_visual_when_degenerate, true);

  nh.param<bool>("visual_map/hybrid_map_en", hybrid_visual_map_en, false);
  nh.param<double>("visual_map/visual_voxel_size", visual_voxel_size, 0.5);
  nh.param<double>("visual_map/local_map_edge_length", visual_local_map_edge_length, 200.0);
  nh.param<double>("visual_map/long_term_map_edge_length", visual_long_term_map_edge_length, 800.0);
  nh.param<double>("visual_map/local_map_sliding_thresh", visual_local_map_slide_thresh, 20.0);
  nh.param<double>("visual_map/long_term_map_sliding_thresh", visual_long_term_map_slide_thresh, 100.0);
  nh.param<int>("visual_map/local_points_per_voxel", visual_local_points_per_voxel, 30);
  nh.param<int>("visual_map/long_term_points_per_voxel", visual_long_term_points_per_voxel, 2);
  nh.param<int>("visual_map/max_obs_size_per_point", visual_max_obs_size_per_point, 30);
  nh.param<int>("visual_map/max_append_points_normal", visual_max_append_points_normal, 600);
  nh.param<int>("visual_map/max_append_points_degenerate", visual_max_append_points_degenerate, 1200);
  nh.param<int>("visual_map/max_long_term_obs_per_point", visual_max_long_term_obs_per_point, 1);
  nh.param<bool>("visual_map/compact_long_term_observations", visual_compact_long_term_observations, true);
  nh.param<int>("visual_map/map_stat_log_interval", visual_map_stat_log_interval, 20);
...
void LIVMapper::initializeComponents() 
{
  ...
  vio_manager->hybrid_map_en = hybrid_visual_map_en;
  vio_manager->visual_voxel_size = visual_voxel_size;
  vio_manager->local_map_edge_length = visual_local_map_edge_length;
  vio_manager->long_term_map_edge_length = visual_long_term_map_edge_length;
  vio_manager->local_map_sliding_thresh = visual_local_map_slide_thresh;
  vio_manager->long_term_map_sliding_thresh = visual_long_term_map_slide_thresh;
  vio_manager->local_points_per_voxel = visual_local_points_per_voxel;
  vio_manager->long_term_points_per_voxel = visual_long_term_points_per_voxel;
  vio_manager->max_obs_size_per_point = visual_max_obs_size_per_point;
  vio_manager->max_append_points_normal = visual_max_append_points_normal;
  vio_manager->max_append_points_degenerate = visual_max_append_points_degenerate;
  vio_manager->max_long_term_obs_per_point = visual_max_long_term_obs_per_point;
  vio_manager->compact_long_term_observations = visual_compact_long_term_observations;
  vio_manager->map_stat_log_interval = visual_map_stat_log_interval;
...
double LIVMapper::rotationDeltaRad(const M3D &R_ref, const M3D &R_cur) const
{
  const M3D R_rel = R_ref.transpose() * R_cur;
  const double cos_theta = std::max(-1.0, std::min(1.0, 0.5 * (R_rel.trace() - 1.0)));
  return std::acos(cos_theta);
}

bool LIVMapper::shouldRunVisualUpdate(double img_time)
{
  if (!adaptive_visual_update_en) return true;

  const bool lidar_degenerate = voxelmap_manager->lidar_is_degenerate_;
  const double sigma_min_norm = voxelmap_manager->lidar_sigma_min_norm_;

  if (!has_visual_keyframe) return true;
  if (force_visual_when_degenerate && lidar_degenerate) return true;

  const double sigma_for_scale = std::max(0.0, std::min(1.0 / 3.0, sigma_min_norm));
  const double adaptive_scale_raw = keyframe_scale_gain * std::sqrt(3.0 * sigma_for_scale);
  const double adaptive_scale =
      std::max(keyframe_scale_min, std::min(keyframe_scale_max, adaptive_scale_raw));

  const double trans_thresh = keyframe_trans_thresh_m * adaptive_scale;
  const double rot_thresh =
      keyframe_rot_thresh_deg * adaptive_scale * (std::acos(-1.0) / 180.0);

  const double delta_p = (_state.pos_end - last_visual_keyframe_pos).norm();
  const double delta_theta = rotationDeltaRad(last_visual_keyframe_rot, _state.rot_end);

  if (delta_p > trans_thresh || delta_theta > rot_thresh) return true;
  if (visual_skip_count >= max_visual_skip_frames) return true;
...
void LIVMapper::handleVIO() 
{
  ...
  vio_manager->setLidarDegenerationState(voxelmap_manager->lidar_is_degenerate_,
                                         voxelmap_manager->lidar_sigma_min_norm_);
  const bool run_visual_update =
      shouldRunVisualUpdate(LidarMeasures.measures.back().vio_time);

  if (!run_visual_update)
  {
    visual_skip_count++;
    visual_skip_total++;
    printf("[ VIO ] Skip visual update (sigma_min_norm: %.6f, degenerate: %d, skipped: %d)\n",
           voxelmap_manager->lidar_sigma_min_norm_,
           voxelmap_manager->lidar_is_degenerate_ ? 1 : 0,
           visual_skip_count);
  ...
  has_visual_keyframe = true;
  visual_skip_count = 0;
  visual_update_count++;
  last_visual_keyframe_pos = _state.pos_end;
  last_visual_keyframe_rot = _state.rot_end;
  last_visual_keyframe_time = LidarMeasures.measures.back().vio_time;
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  auto sampleAndFilterXYZI = [&](const PointCloudXYZI::Ptr &cloud_in) -> PointCloudXYZI::Ptr
  {
    PointCloudXYZI::Ptr sampled(new PointCloudXYZI());
    if (cloud_in == nullptr || cloud_in->empty()) return sampled;

    if (max_publish_points > 0 && static_cast<int>(cloud_in->size()) > max_publish_points)
    {
      const size_t step = std::max<size_t>(1, cloud_in->size() / static_cast<size_t>(max_publish_points));
      sampled->reserve(static_cast<size_t>(max_publish_points));
      for (size_t i = 0; i < cloud_in->size() && static_cast<int>(sampled->size()) < max_publish_points; i += step)
      {
        sampled->push_back(cloud_in->points[i]);
      }
    }
    else
    {
    ...
    if (publish_voxel_leaf_size > 1e-6 && !sampled->empty())
    {
      pcl::VoxelGrid<PointType> voxel_filter;
      voxel_filter.setLeafSize(publish_voxel_leaf_size, publish_voxel_leaf_size, publish_voxel_leaf_size);
      voxel_filter.setInputCloud(sampled);
  ...
  auto sampleAndFilterXYZRGB = [&](const PointCloudXYZRGB::Ptr &cloud_in) -> PointCloudXYZRGB::Ptr
  {
    PointCloudXYZRGB::Ptr sampled(new PointCloudXYZRGB());
    if (cloud_in == nullptr || cloud_in->empty()) return sampled;
  ...
  bool vio_batch_ready = true;

  if (LidarMeasures.lio_vio_flg == VIO)
  {
    pub_num++;
    const int publish_stride = std::max(1, pub_scan_num);
    vio_batch_ready = pub_num >= publish_stride;
    *pcl_wait_pub += *pcl_w_wait_pub;
    if (vio_batch_ready)
    {
      ...
      if (vio_colorize_en)
      {
        ...
        size_t step = 1;
        if (max_publish_points > 0 && static_cast<int>(size) > max_publish_points)
        {
          step = std::max<size_t>(1, size / static_cast<size_t>(max_publish_points));
        }

        laserCloudWorldRGB->reserve(std::max<size_t>(1, size / step));
  ...
  if (LidarMeasures.lio_vio_flg == VIO && !vio_batch_ready) return;
  ...
  if (slam_mode_ == LIVO && LidarMeasures.lio_vio_flg == VIO)
  {
    if (vio_colorize_en)
    {
      PointCloudXYZRGB::Ptr cloud_pub = sampleAndFilterXYZRGB(laserCloudWorldRGB);
      pcl::toROSMsg(*cloud_pub, laserCloudmsg);
    }
    else
    {
      PointCloudXYZI::Ptr cloud_pub = sampleAndFilterXYZI(pcl_wait_pub);
      pcl::toROSMsg(*cloud_pub, laserCloudmsg);
    }
  }
  if (slam_mode_ == ONLY_LIO || slam_mode_ == ONLY_LO)
  { 
    PointCloudXYZI::Ptr cloud_pub = sampleAndFilterXYZI(pcl_w_wait_pub);
    pcl::toROSMsg(*cloud_pub, laserCloudmsg); 
  }
  ...
  if (pcd_save_en)
  {
    ...
    switch (pcd_save_type)
    {
      case 0: /** world frame **/
        if (slam_mode_ == LIVO)
        {
          if (vio_colorize_en && !laserCloudWorldRGB->empty()) *pcl_wait_save += *laserCloudWorldRGB;
          else *pcl_wait_save_intensity += *pcl_wait_pub;
        }
        else
        {
