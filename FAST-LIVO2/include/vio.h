...
public:
  ...
  // ---- VIO skip-frame gating parameters ----
  int    skip_frame_num_       = 0;      // skip N frames between VIO updates (0 = disabled)
  double vio_delta_p_thresh_   = 0.05;   // [m]   position delta to trigger VIO
  double vio_delta_theta_thresh_ = 0.05; // [rad] rotation delta to trigger VIO
  bool   publish_frame_world_flag_ = true; // set false when frame is skipped

private:
  // ---- internal state for gating ----
  int  process_frame_count_ = 0;
  V3D  last_vio_pos_        = V3D::Zero();
  M3D  last_vio_rot_        = M3D::Identity();
