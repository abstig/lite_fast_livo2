...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("vio/max_iterations", max_iterations, 2); // 针对Nano轻量化，原为5
  nh.param<double>("vio/img_point_cov", IMG_POINT_COV, 100);
  nh.param<bool>("vio/raycast_en", raycast_en, false);
  nh.param<bool>("vio/exposure_estimate_en", exposure_estimate_en, false); // 针对Nano关闭曝光估计
  nh.param<double>("vio/inv_expo_cov", inv_expo_cov, 0.2);
  nh.param<int>("vio/grid_size", grid_size, 30);          // 增大网格，降低特征数量
  nh.param<int>("vio/grid_n_height", grid_n_height, 17);
  nh.param<int>("vio/patch_pyrimid_level", patch_pyrimid_level, 2); // 削减金字塔层数
  nh.param<int>("vio/patch_size", patch_size, 6);         // 减小光流追踪Patch尺寸
  ...
  nh.param<double>("preprocess/filter_size_surf", filter_size_surf_min, 0.8); // 增大体素降采样
  nh.param<bool>("preprocess/hilti_en", hilti_en, false);
  nh.param<int>("preprocess/lidar_type", p_pre->lidar_type, AVIA);
  nh.param<int>("preprocess/scan_line", p_pre->N_SCANS, 6);
  nh.param<int>("preprocess/point_filter_num", p_pre->point_filter_num, 4); // 更强的前端降采样
  ...
  nh.param<bool>("publish/dense_map_en", dense_map_en, false); // 不发布稠密全云
