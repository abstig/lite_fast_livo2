...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("vio/skip_frame_num", skip_frame_num, 0);
  nh.param<double>("vio/vio_delta_p_thresh", vio_delta_p_thresh, 0.05);
  nh.param<double>("vio/vio_delta_theta_thresh", vio_delta_theta_thresh, 0.05);
...
void LIVMapper::initializeComponents() 
{
  ...
  // Set VIO skip-frame gating parameters
  vio_manager->skip_frame_num_ = skip_frame_num;
  vio_manager->vio_delta_p_thresh_ = vio_delta_p_thresh;
  vio_manager->vio_delta_theta_thresh_ = vio_delta_theta_thresh;
...
void LIVMapper::handleVIO() 
{
  ...
  // Only publish coloured point cloud when a full VIO update was performed
  if (vio_manager->publish_frame_world_flag_)
  {
