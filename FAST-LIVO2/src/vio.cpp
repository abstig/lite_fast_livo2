...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  // ---- VIO skip-frame gating ----
  process_frame_count_++;
  bool do_vio = false;

  if (process_frame_count_ == 1)
  {
    // Always process the very first frame
    do_vio = true;
  }
  else if (skip_frame_num_ > 0 && (process_frame_count_ % (skip_frame_num_ + 1) == 1))
  {
    // Fixed frame-skip: process every (skip_frame_num_+1)-th frame
    do_vio = true;
  }
  else
  {
    // Pose-delta gating (used when skip_frame_num_==0 or as extra condition)
    V3D cur_pos = state->pos_end;
    M3D cur_rot = state->rot_end;
    double delta_p = (cur_pos - last_vio_pos_).norm();
    double trace_val = (cur_rot * last_vio_rot_.transpose()).trace();
    double delta_theta = (trace_val > 3.0 - 1e-6) ? 0.0 : std::acos(0.5 * (trace_val - 1.0));
    if (delta_p > vio_delta_p_thresh_ || delta_theta > vio_delta_theta_thresh_)
    {
      do_vio = true;
    }
  }
  ...
  if (!do_vio)
  {
    printf("[ VIO ] Skip frame %d (no gating threshold met)\n", process_frame_count_);
    plotTrackedPoints();
    publish_frame_world_flag_ = false; // signal caller that no full update happened
    return;
  }

  // Record pose at which we do VIO
  last_vio_pos_ = state->pos_end;
  last_vio_rot_ = state->rot_end;
  publish_frame_world_flag_ = true;
  // ---- end gating ----
